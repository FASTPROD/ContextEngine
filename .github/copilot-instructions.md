...
| `src/fleet-health.ts` | Measured fleet health (version drift, reindex rate, blocks, refusals, last verified release, Claude Code hook registrations per event and doubled hook events today) written by the indexer to `~/.contextengine/fleet-health.json`; read by `servers`, `end-session` and the VS Code status bar |
...
17. **NEVER `git push` in another project's repo** — `git push` publishes the ENTIRE branch, not the commit you just made. Fleet-wide operations (score regeneration, doc sweeps) may commit a single named file per repo when the user approves that scope, but pushing is a separate act needing separate justification. Every repo in this workspace already auto-pushes via `.git/hooks/post-commit` — **commit and stop**. If you believe an explicit push is needed, first run `git rev-list --count @{u}..HEAD` and require it to equal the number of commits YOU made; anything higher is the user's unpublished work — STOP and ask. Treat a missing upstream (`@{u}` fails) the same way. **Origin:** 2026-08-08 — a SCORE.md fleet pass pushed two of the user's deliberately-unpushed commits in `shop.invoc.io` (`7330a77`, `3dd3af4`) onto origin. The commit half was correctly scoped to one file; the push half published everything. See learnings `mskm72y2hwku` + `mskm7c07hozv`.
18. **Absence must never be encoded as a decision** — if a check cannot determine its condition, it emits `unknown` (0 points, ❔, listed under "Could Not Be Verified"), never `pass` or `fail`. Every pass/fail detail must name what was inspected. Watch for stdlib functions whose "not found" answer absorbs "could not look": `existsSync()` follows symlinks so a dangling link reads as absent, and any `catch { return "" }` makes a failed command indistinguishable from a clean empty result. Both shipped real bugs here — a hook row that said "consider auto-push" about a hook that existed but was broken, and a security check that awarded 6/6 precisely when `git ls-files` could not run. Before adding a `catch` that returns a falsy default, ask what the caller concludes from it. LOCKs `[ABSENCE-IS-NOT-A-VERDICT]`, `[EXEC-FAILURE-IS-NOT-EMPTY]`.
19. **The canary gates every scoring run** — `runScoreCanary()` must pass before `cliScore` writes a single `SCORE.md`. Do NOT downgrade it to a warning, do NOT narrow it to test one known defect, and do NOT treat "fixture could not be built" as a pass. It is the only check here that catches failures nobody predicted — named guards only stop repeats. It found `[EXEC-FAILURE-IS-NOT-EMPTY]` on its first execution. LOCK `[SCORE-CANARY]`.
20. **Fleet scripts: no `timeout`, no `${=var}`** — `timeout` does not exist on macOS (it is `gtimeout` from coreutils, if installed); a script using it fails every wrapped command while reporting the shell's error as the tool's error. zsh's `${=var}` word-splits on spaces and shatters paths — this workspace has `GOOGLE Analytics`, `STRIPE backend`, and `Flutter crowler`. Keep paths in real arrays; set `GIT_TERMINAL_PROMPT=0` so credential prompts fail fast instead of hanging. See learning `mskm7x095abg`.
21. **NEVER skip CE checks because "the task is done"** — Completing the user's explicit request is NOT the end of the task. Agents have a documented bias: they mark todos as "completed" after pushing code and mentally treat the work as finished, skipping `end_session`, `score_project`, and doc updates. This is the #1 compliance failure mode. The pattern "commit → push → stop" is ALWAYS wrong. The correct pattern is "commit → push → `end_session` → verify → fix anything it catches → re-commit if needed." Treat `end_session` as automatic as `git push` itself — not something the user has to ask for.
