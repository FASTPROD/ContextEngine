...
- **SSH**: Password auth — see `.copilot-credentials.md` (local, gitignored) for the SSH command. SSH key passphrase is lost; password auth is used.
...
10. **SSH to Gandi VPS** — Credentials stored in `.copilot-credentials.md` (local, gitignored). SSH key passphrase is lost; use password auth with `sshpass`. For rsync: exclude `node_modules/`, `data/`, `delta-modules/`.
