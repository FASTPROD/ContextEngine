...
### H3. Publisher identity needs structured DISCLOSURE — audit's "consolidate" recommendation was wrong-headed; corrected + shipped 2026-06-24
**Lenses:** trust-signals (Findings 32, 36, 38)
**Files:** `chrome-extension/src/manifest.json`, `chrome-extension/src/options/options.html` (footer), `README.md` (Publisher section), `vscode-extension/package.json`, `package.json`, `docs/about.md` (new)
**Root cause (corrected understanding per user clarification, 2026-06-23):** The names are NOT chaos — they describe a real corporate hierarchy. **CSS LLC** is a Swiss company incorporated in 2005 = the legal parent. **PROD LLC** is its operating brand. **FASTPROD, CROWLR, KONIVE, INVOC** are product brands of PROD LLC. **compR.fr / compR.app** are portfolio sites. The original audit recommendation to "pick ONE legal entity" would require changing the actual legal-entity registration, which is wrong-headed. The trust-collapse signal a fresh user gets IS real, but the fix is structured **disclosure** of the hierarchy, not consolidation. The VS Code Marketplace publisher `css-llc` maps to the legal parent and is CORRECT — keep as-is.
**What was done (2026-06-24):**
  - **`chrome-extension/src/manifest.json`** — added `"author"` naming PROD LLC + CSS LLC + incorporation year + contact. Added `"homepage_url"` pointing at `docs/about.md`.
  - **`chrome-extension/src/options/options.html`** footer — "Built by **PROD LLC** — an operating brand of **CSS LLC** (Swiss company, incorporated 2005). Source-available under BSL-1.1." with About + GitHub + email links.
  - **`README.md`** — replaced bare "Built by [PROD LLC]" + 7-domain row with a structured `## Publisher` section: names CSS LLC as legal parent, explains the marketplace publisher ID, tabulates sibling brands with domains + one-line descriptions, links to About.
  - **`docs/about.md`** (NEW, canonical) — legal entity stack, marketplace IDs, sibling brands table, what OpsContext is and is NOT (single team, local-first, NOT SOC 2 certified, NOT part of a larger cloud platform), contact channels, 2005-to-2026 timeline. Linked from manifest `homepage_url`, Options footer, README, this audit doc.
  - **`vscode-extension/package.json`** — added `author` field naming PROD LLC + CSS LLC + email + About-page URL.
  - **`package.json`** — `author.name` changed from "FASTPROD (PROD LLC)" (confusing — FASTPROD is a brand, not the entity) to "PROD LLC (operating brand of CSS LLC, Swiss, est. 2005)".
  - **NOT changed (verified correct):** VS Code Marketplace `publisher: css-llc` — that maps to the legal parent and is the right value. npm scope `@compr` — that's the portfolio domain, owned by CSS LLC.
**Open polish (carry-forward):** chrome-ext `PRIVACY.md` § 7 still lists only `yannick@compr.ch` for contact — add a one-line "publisher: PROD LLC, see About" pointer too. compr.fr portfolio site itself still lists the OLD ContextEngine card with `@compr/contextengine-mcp` + "942 quality learnings" pre-rebrand copy — separate repo at `/Users/yan/Projects/compR.fr` and needs its own update.
