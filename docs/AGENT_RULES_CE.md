...
| After `npm publish`, `npm run verify-release` must pass before the word "published" is written anywhere (since 2026-09-06 a Stop hook in this repo, `scripts/release-gate.sh`, refuses to end the turn until it has): a real MCP `initialize` + `tools/list` from the repo build AND from the registry package in a foreign cwd, served version compared to package.json. `npm view`, a tarball grep or `--version` prove nothing about Claude Code. | Three releases were "published and verified" while Claude Code ran nothing here (`sh: opscontext: command not found` in its MCP log, 2026-08-23 and 2026-09-05). | `scripts/verify-release.sh`, LOCK `[RELEASE-PROVEN-BY-HANDSHAKE]`. |
...
- Session save is a gate, not a reminder (2026-09-06, 2.7.0): `contextengine session-gate` runs as
  a Claude Code Stop hook at user scope (`contextengine install-claude-hook`). A turn cannot end
  while the CE session for the repo is older than HEAD; the message names the session to save,
  the session doc, and how far copilot-instructions is behind. Save the session after every
  commit. Anything built for the fleet that a customer would also want goes into the package.
