...
- **Rotation moves history, it never deletes it** (v2.5.2, `[ROTATION-MUST-NOT-ORPHAN-THE-CHAIN]`). `contextengine audit-rotate [--keep-days N] [--max-records N] [--dry-run]` moves a prefix of the log into a numbered, append-only segment under `~/.contextengine/audit-archive/audit-NNNN.jsonl`. The canonical history is **segments in order, then the live log**, so `readAuditLog()` and therefore `audit-verify` see one continuous chain. A naive `mv audit.log audit.log.1` would make the first record of the new file an **orphan**, which the verifier correctly reports as deleted history: that single fact is why rotation had to be built rather than scripted.
  - **Two cut rules, whichever archives more** (`[DATE-RETENTION-DOES-NOT-BOUND-SIZE]`). Age alone does not bound size: measured at 80,000 records/day, a 30-day window still left 390,445 records live. The record ceiling is what caps the file. A floor of 2,000 live records is always kept.
  - **Archive first, truncate second** (`[ROTATE-ARCHIVE-BEFORE-TRUNCATE]`). The only reachable crash state is a duplicated prefix, never a lost one; `readAuditLog()` drops the leading duplicate run at the seam.
  - **Refuses to run when the live log does not verify** (`[ROTATE-REFUSES-LIVE-DAMAGE-ONLY]`, 2026-08-21; it used to veto on any damage anywhere). Damage already inside an archived segment is reported on stderr and does not block: those segments are immutable by policy, and a permanent veto let the live log reach 170k records. Forks are fine, they are concurrency.
  - **Runs by itself at MCP startup** (`[AUTO-ROTATE-HYSTERESIS-AND-ONE-RUNNER]`, 2026-08-21). `autoRotateAuditLog()` fires 3 s after the transport connects: trigger at 100k live records (2x the 50k ceiling, so one rotation buys about a day), rotate down to the ceiling, dedicated `audit.rotate.lock` (O_EXCL, stale after 10 min) so the three servers that start together on one machine do not rotate the same file. `CONTEXTENGINE_AUTO_ROTATE=0` disables it.
  - **The rotation audits itself**: an `audit.rotate` record naming the segment, its record count and its first/last hash is appended to the new live log.
  - **Why it matters beyond disk**: the drift detectors call `scanRecentEvents()` for a 5-minute window on every tick. That read is live-log-only, so on the author's machine it went from **2,386 ms / 693 MB to 121 ms** once 494,152 records moved to a segment.
- **A redaction is a chained record, never an allow-list** (`[REDACTION-IS-A-CHAINED-RECORD]`, 2026-08-21). Removing a secret from a record after the fact is legitimate and the hash chain can only see it as "content no longer matches": on 2026-08-20 a credentials sweep did exactly that to 3 archived records, and `audit-verify` said tampering, truthfully. `contextengine audit-redact-ack --index i,j --reason "..."` appends an `audit.redact` record binding each original hash to the hash of the redacted content. The verifier then reports those records as **redacted** (counted separately, `ok` ignores them) instead of altered. Only actually altered records can be acknowledged, an acknowledgement cannot itself be acknowledged, and a further edit breaks the binding. An out-of-band list would have the same effect on the wording and none of the evidence: whoever can edit the log could edit the list.
